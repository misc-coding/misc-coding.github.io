...
test("all dashboard controls update their panels without runtime errors", async () => {
  ...
  assert.ok(document.querySelector("#temporal-init-select").options.length === 6);
  ...
  assert.equal(document.querySelector("#imerg-time-select").options.length, 288);
  ...
  const zoomStart = document.querySelector("#imerg-zoom-start");
  const zoomEnd = document.querySelector("#imerg-zoom-end");
  assert.ok(zoomStart.options.length >= 3);
  assert.equal(zoomStart.options.length, zoomEnd.options.length);
  assert.ok(document.querySelector("[data-imerg-zoom-surface]"));
  const fullZoomCount = zoomStart.options.length;
  const fullChart = document.querySelector("#imerg-validation-chart svg").outerHTML;
  zoomStart.value = zoomStart.options[1].value;
  zoomStart.dispatchEvent(new window.Event("change", { bubbles: true }));
  const refreshedEnd = document.querySelector("#imerg-zoom-end");
  refreshedEnd.value = refreshedEnd.options[refreshedEnd.options.length - 2].value;
  refreshedEnd.dispatchEvent(new window.Event("change", { bubbles: true }));
  assert.match(document.querySelector("#imerg-validation-summary").textContent, new RegExp(`showing ${fullZoomCount - 2} of ${fullZoomCount}`));
  assert.notEqual(document.querySelector("#imerg-validation-chart svg").outerHTML, fullChart);
  assert.match(window.location.search, /imerg_zoom_start=/);
  document.querySelector("#imerg-zoom-reset").click();
  assert.match(document.querySelector("#imerg-validation-summary").textContent, new RegExp(`showing ${fullZoomCount} of ${fullZoomCount}`));
  const zoomSurface = document.querySelector("[data-imerg-zoom-surface]");
  zoomSurface.ownerSVGElement.getBoundingClientRect = () => ({ left: 0, top: 0, width: 1000, height: 430, right: 1000, bottom: 430 });
  zoomSurface.dispatchEvent(new window.MouseEvent("pointerdown", { clientX: 180, bubbles: true }));
  zoomSurface.dispatchEvent(new window.MouseEvent("pointermove", { clientX: 620, bubbles: true }));
  zoomSurface.dispatchEvent(new window.MouseEvent("pointerup", { clientX: 620, bubbles: true }));
  assert.doesNotMatch(document.querySelector("#imerg-validation-summary").textContent, new RegExp(`showing ${fullZoomCount} of ${fullZoomCount}`));
  document.querySelector("#imerg-zoom-reset").click();
